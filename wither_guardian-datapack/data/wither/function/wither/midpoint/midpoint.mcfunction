...
# BEDROCK
execute if data storage wither:options {toggleanimation:Bedrock} as @e[type=wither,tag=WitherGuardian,limit=1,sort=nearest] run data merge entity @e[type=wither,tag=WitherGuardian,limit=1,sort=nearest] {Motion:[0.0,-10.0,0.0]}
execute if data storage wither:options {toggleanimation:Bedrock} as @e[type=wither,tag=WitherGuardian,limit=1,sort=nearest] run schedule function wither:wither/midpoint/midpointbedrock 10t

# GO UP GO DOWN 
execute if data storage wither:options {toggleanimation:Default} at @e[type=wither,tag=WitherGuardian,limit=1,sort=nearest] align xyz positioned ~.5 ~.5 ~.5 run summon armor_stand ~ ~ ~ {Invisible:1b,Tags:["midpointpos"],attributes:[{id:"minecraft:scale",base:2}]}

# BREAK BOX + SFX
execute as @e[type=minecraft:wither,tag=WitherGuardian,limit=1,sort=nearest] if data storage wither:options {toggleanimation:Default} if data storage wither:options {toggledestruction:Enabled} run fill ~-1 ~ ~-1 ~1 ~3 ~1 air replace
execute as @e[type=minecraft:wither,tag=WitherGuardian,limit=1,sort=nearest] at @s run playsound minecraft:entity.wither.break_block block @a[distance=..30] ~ ~ ~ 0.5 .1
execute at @e[type=minecraft:wither,tag=WitherGuardian,limit=1,sort=nearest] if data storage wither:options {toggleanimation:Default} run playsound minecraft:entity.wither_skeleton.hurt hostile @a[distance=..30] ~ ~ ~ 0.5 0.1
