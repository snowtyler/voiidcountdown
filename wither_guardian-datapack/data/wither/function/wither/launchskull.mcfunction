...
execute as @e[type=minecraft:wither,tag=WitherGuardian,limit=1] at @s run rotate @s facing entity @p eyes
execute as @e[type=minecraft:wither,tag=WitherGuardian] at @s run playsound minecraft:entity.wither.shoot player @e[type=player,distance=..50] ~ ~ ~ 0.3 1.2
execute as @e[type=minecraft:wither,tag=WitherGuardian,limit=1,sort=nearest] at @s run summon minecraft:wither_skull ^ ^3 ^ {Tags:["skulllaunch"],Motion:[0.05,0.09,0.05],Fire:200,CustomName:"Wither Missile"}
