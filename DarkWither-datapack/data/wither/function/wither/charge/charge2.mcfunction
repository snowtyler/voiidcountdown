...
execute as @e[type=minecraft:wither,tag=DarkWither,limit=1,sort=nearest] at @s if data storage wither:options {toggledestruction:Disabled} run tp @s @e[type=armor_stand,limit=1,tag=chargew]
execute as @e[type=minecraft:wither,tag=DarkWither,limit=1,sort=nearest] at @s run kill @e[type=armor_stand,tag=chargew]
kill @e[type=armor_stand,tag=midpointpos]
data merge entity @e[type=minecraft:wither,tag=DarkWither,limit=1,sort=nearest] {Glowing:0b}
