...
execute if score LanguageTimer Timer matches 1 run function voiidtimer:reset
execute if score LanguageTimer Timer matches 1 run tellraw @s ["-------------------------------------\n",{"text":"Voiid Countdown Timer","color":"light_purple","bold":true}," ",{"text":"has been reloaded to\nchange the language, use or click the command\nto get the book:","color":"green"},"\n",{"text":"/function voiidtimer:book","bold":true,"clickEvent":{"action":"run_command","value":"/function voiidtimer:book"},"hoverEvent":{"action":"show_text","contents":"Click here to execute this command!"},"color":"aqua"},"\n-------------------------------------"]
